...
class _CompleteTaskScreenState extends State<CompleteTaskScreen> {
  ...
  Future<void> _complete() async {
    ...
    if (_items.where((i) => i.weightKg <= 0).isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    final alreadyDone = err.toLowerCase().contains('tidak valid') ||
        err.toLowerCase().contains('invalid') || err.toLowerCase().contains('status');

    if (success || alreadyDone) {
      _showSuccessAndRedirect();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(err.isNotEmpty ? err : 'Gagal'), backgroundColor: AppColors.error));
    }
  }
        ...
        builder: (ctx, setDialogState) {
          ...
          return AlertDialog(
            ...
            content: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(height: 8),
              Text(
                '$_totalPoints poin dari ${_totalWeight.toStringAsFixed(2)} kg telah diberikan ke user.',
                style: AppTextStyles.bodySmall, textAlign: TextAlign.center,
              ),
              const SizedBox(height: 16),
              Text('Kembali ke dashboard dalam $countdown detik...',
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Container(
                      padding: const EdgeInsets.all(16),
                      decoration: BoxDecoration(color: AppColors.collectorSurface, borderRadius: BorderRadius.circular(16)),
                      child: Row(children: [
                        const Icon(Icons.info_outline, color: AppColors.collectorPrimary),
                        const SizedBox(width: 12),
                        Expanded(child: Text(
                          'Poin dihitung otomatis berdasarkan berat dan kategori.',
                    ...
                    if (_items.isEmpty)
                      ...
                      ...List.generate(_items.length, (i) => _WasteItemCard(
                        item: _items[i], categories: provider.categories,
                        index: i + 1,
...
class _WasteItem {
  String categoryId; WasteCategory category; double weightKg;
...
class _WasteItemCard extends StatefulWidget {
  final _WasteItem item; final List<WasteCategory> categories;
  final int index; final VoidCallback onDelete; final VoidCallback onChanged;
  ...
  @override State<_WasteItemCard> createState() => _WasteItemCardState();
}

class _WasteItemCardState extends State<_WasteItemCard> {
  late TextEditingController _weightCtrl;
  @override void initState() { super.initState(); _weightCtrl = TextEditingController(text: widget.item.weightKg > 0 ? widget.item.weightKg.toString() : ''); }
  @override void dispose() { _weightCtrl.dispose(); super.dispose(); }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const Spacer(),
          GestureDetector(onTap: widget.onDelete, child: Container(
        ...
        DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            labelText: 'Kategori', filled: true, fillColor: AppColors.surfaceVariant,
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
            contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
          ),
          items: widget.categories.map((cat) => DropdownMenuItem(
            value: cat.id, child: Text('${cat.name} (${cat.pointsPerKg} poin/kg)', style: AppTextStyles.bodySmall),
          )).toList(),
          onChanged: (v) { if (v != null) { widget.item.categoryId = v; widget.item.category = widget.categories.firstWhere((c) => c.id == v); widget.onChanged(); } },
        ),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: TextFormField(
            ...
            decoration: InputDecoration(
              labelText: 'Berat (kg)', suffixText: 'kg', filled: true, fillColor: AppColors.surfaceVariant,
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
              contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
            ),
            onChanged: (v) { widget.item.weightKg = double.tryParse(v) ?? 0; widget.onChanged(); },
          )),
          const SizedBox(width: 12),
          Container(
